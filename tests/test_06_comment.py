...
from .common import auth_client, create_Comment, create_reviews
...
class Test06CommentAPI:
    ...
    def test_01_comment_not_auth(self, client, user_client, admin):
        ...
        response = client.get(f'/api/v1/titles/{titles[0]["id"]}/reviews/{reviews[0]["id"]}/Comment/')
        assert response.status_code != 404, \
            'Страница `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/` ' \
            'не найдена, проверьте этот адрес в *urls.py*'
        assert response.status_code == 200, \
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/` ' \
            'без токена авторизации возвращается статус 200'

    def create_comment(self, client_user, title_id, review_id, text):
        data = {'text': text}
        response = client_user.post(f'/api/v1/titles/{title_id}/reviews/{review_id}/Comment/', data=data)
        assert response.status_code == 201, \
            'Проверьте, что при POST запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/` ' \
    ...
    def test_02_comment(self, user_client, admin):
        ...
        data = {}
        response = user_client.post(f'/api/v1/titles/{titles[0]["id"]}/reviews/{reviews[0]["id"]}/Comment/', data=data)
        ...
        data = {'text': 'kjdfg'}
        response = user_client.post(f'/api/v1/titles/999/reviews/999/Comment/', data=data)
        assert response.status_code == 404, \
            'Проверьте, что при POST запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/` ' \
        ...
        response = user_client.get(f'/api/v1/titles/{titles[0]["id"]}/reviews/{reviews[0]["id"]}/Comment/')
        assert response.status_code == 200, \
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/` ' \
        ...
        for item in data['results']:
            if item.get('text') == 'qwerty':
                comment = item
        assert comment, 'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/` ' \
    ...
    def test_03_review_detail(self, client, user_client, admin):
        Comment, reviews, titles, user, moderator = create_Comment(user_client, admin)
        pre_url = f'/api/v1/titles/{titles[0]["id"]}/reviews/{reviews[0]["id"]}/Comment/'
        response = client.get(f'{pre_url}{Comment[0]["id"]}/')
        assert response.status_code != 404, \
            'Страница `/api/v1/titles/{title_id}/reviews/{review_id}/{review_id}/Comment/{comment_id}/` ' \
            'не найдена, проверьте этот адрес в *urls.py*'
        assert response.status_code == 200, \
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/{comment_id}/` ' \
            'без токена авторизации возвращается статус 200'
        data = response.json()
        ...
        data = {'text': 'rewq'}
        response = user_client.patch(f'{pre_url}{Comment[0]["id"]}/', data=data)
        assert response.status_code == 200, \
            'Проверьте, что при PATCH запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/{comment_id}/` ' \
            'возвращается статус 200'
        data = response.json()
        assert data.get('text') == 'rewq', \
            'Проверьте, что при PATCH запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/{comment_id}/` ' \
            'возвращаете данные объекта. Значение `text` изменено.'
        response = user_client.get(f'{pre_url}{Comment[0]["id"]}/')
        assert response.status_code == 200, \
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/{comment_id}/` ' \
            'без токена авторизации возвращается статус 200'
        data = response.json()
        ...
        client_user = auth_client(user)
        data = {'text': 'fgf'}
        response = client_user.patch(f'{pre_url}{Comment[2]["id"]}/', data=data)
        assert response.status_code == 403, \
            'Проверьте, что при PATCH запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/{comment_id}/` ' \
            'от обычного пользователя при попытки изменить не свой отзыв возвращается статус 403'

        data = {'text': 'jdfk'}
        response = client_user.patch(f'{pre_url}{Comment[1]["id"]}/', data=data)
        assert response.status_code == 200, \
            'Проверьте, что при PATCH запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/{comment_id}/` ' \
            'возвращается статус 200'
        data = response.json()
        ...
        client_moderator = auth_client(moderator)
        response = client_moderator.delete(f'{pre_url}{Comment[1]["id"]}/')
        assert response.status_code == 204, \
            'Проверьте, что при DELETE запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/{comment_id}/` ' \
            'возвращаете статус 204'
        response = user_client.get(f'{pre_url}')
        test_data = response.json()['results']
        assert len(test_data) == len(Comment) - 1, \
            'Проверьте, что при DELETE запросе `/api/v1/titles/{title_id}/reviews/{review_id}/Comment/{comment_id}/` ' \
            'удаляете объект'
    ...
    def test_04_comment_check_permission(self, client, user_client, admin):
        ...
        assert response.status_code == 401, \
            f'Проверьте, что при POST запросе `/api/v1/titles/{{title_id}}/reviews/{{review_id}}/Comment/` ' \
            f'без токена авторизации возвращается статус 401'
        response = client.patch(f'{pre_url}{Comment[1]["id"]}/', data=data)
        assert response.status_code == 401, \
            f'Проверьте, что при PATCH запросе `/api/v1/titles/{{title_id}}/reviews/{{review_id}}/Comment/{{comment_id}}/` ' \
            f'без токена авторизации возвращается статус 401'
        response = client.delete(f'{pre_url}{Comment[1]["id"]}/')
        assert response.status_code == 401, \
            f'Проверьте, что при DELETE запросе `/api/v1/titles/{{title_id}}/reviews/{{review_id}}/Comment/{{comment_id}}/` ' \
            f'без токена авторизации возвращается статус 401'
        self.check_permissions(user, 'обычного пользователя', f'{pre_url}{Comment[2]["id"]}/')
